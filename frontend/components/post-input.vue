<template lang="">
  <div id="create-status" class="hidden lg:p-20 uk- open" uk-modal="">
    <div
    ...
    >
      ...
      <form ref="post_form" action="" @submit="handleSubmitPost">
        <div class="space-y-5 mt-3 p-2">
          <textarea
            id=""
            class="w-full !text-black placeholder:!text-black !bg-white !border-transparent focus:!border-transparent focus:!ring-transparent !font-normal !text-xl   dark:!text-white dark:placeholder:!text-white dark:!bg-slate-800" name="post-content-text" rows="6" placeholder="What do you have in mind?" required
          />
        </div>
        ...
        <div class="flex items-center gap-2 text-sm py-2 px-4 font-medium flex-wrap">
          <button
            ...
            @click="openFileInput"
          >
            <ion-icon name="image" class="text-base" />
            Image
          </button>
          <input
            id="photo-input" ref="fileInput" type="file" name="photo" accept="image/*"
            style="display: none"
          >
        </div>

        <UAlert v-if="showAlert" icon="ic:baseline-warning" class="custom-alert" variant="solid" title="Error!" description="Choose audiance for your post " />
        ...
        <div class="p-5 flex justify-between items-center">
          <div>
            ...
            >
              ...
              <label>
                <input
                  ref="publicCheck" type="radio" name="radio-status"
              ...
              <label>
                <input
                  ref="privateCheck" type="radio" name="radio-status"
...
export default {
    ...
    methods: {
        ...
        async handleSubmitPost(e) {
            e.preventDefault()
            const followersSelected = Array.from($('.js-example-basic-multiple').find(':selected'))
            ...
            try {
                const response = await fetch('http://localhost:8081/post/insert', {
                    method: 'POST',
                    headers: {
                        Authorization: `Bearer ${useGlobalAuthStore().token}`
                    },
